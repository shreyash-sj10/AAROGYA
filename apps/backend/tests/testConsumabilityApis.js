...
(async () => {
  ...
  assert(Array.isArray(routes["/assistant/orchestrate"]), "Expected /assistant/orchestrate route to be registered");
  ...
  const orchestrateInvalidReq = {
    body: {
      user_input: "Give me a diet plan",
      context: {},
    },
  };
  const orchestrateInvalidRes = createMockRes();
  await executeHandlers(routes["/assistant/orchestrate"], orchestrateInvalidReq, orchestrateInvalidRes);
  assert(orchestrateInvalidRes.statusCode === 200, "Expected /assistant/orchestrate invalid request to return 200");
  assert(orchestrateInvalidRes.body && orchestrateInvalidRes.body.intent === "INVALID_REQUEST", "Expected INVALID_REQUEST intent");
  assert(
    orchestrateInvalidRes.body && orchestrateInvalidRes.body.message === "Meal plans are generated by the system. Ask for explanation instead.",
    "Expected deterministic invalid-request message"
  );

  const orchestrateSymptomsReq = {
    body: {
      user_input: "I feel acidity and heat",
      context: {
        allowed_labels: ["high_pitta", "high_vata"],
      },
    },
  };
  const orchestrateSymptomsRes = createMockRes();
  await executeHandlers(routes["/assistant/orchestrate"], orchestrateSymptomsReq, orchestrateSymptomsRes);
  assert(orchestrateSymptomsRes.statusCode === 200, "Expected /assistant/orchestrate symptoms request to return 200");
  assert(orchestrateSymptomsRes.body && orchestrateSymptomsRes.body.intent === "INTERPRET_SYMPTOMS", "Expected INTERPRET_SYMPTOMS intent");
  assert(orchestrateSymptomsRes.body && orchestrateSymptomsRes.body.routed_endpoint === "/profile", "Expected /profile route mapping");
