...
async function runTest(testName, config, validate) {
  ...
  console.log(JSON.stringify({
    testName,
    meal: (result.meal_plan || []).map((item) => item.name || item.recipe_id),
    score: result.score,
...
(async () => {
  await runTest(
    ...
    (result, detailed) => {
      ...
      assert(((result.confidence || {}).value || 0) > 0.5, "Confidence should be greater than 0.5");
    }
  );
  ...
  await runTest(
    ...
    (result) => {
      const mealNames = Array.isArray(result.meal_plan)
        ? result.meal_plan.map((entry) => (entry && (entry.name || entry.recipe_id) ? String(entry.name || entry.recipe_id) : ""))
        : [];
      const deterministicExplanation = result.explanation && typeof result.explanation === "object"
        ? String(result.explanation.deterministic || "")
        : "";
      assert((result.score || 0) <= 0.1, "Fallback meal should be triggered");
      assert(((result.confidence || {}).value || 1) <= 0.5, "Fallback confidence should be low");
      assert(deterministicExplanation.length > 0 || mealNames.length > 0, "Fallback response should remain explainable");
    }
  );
