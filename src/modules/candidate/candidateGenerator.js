...
const { filterFoods } = require("../rules/constraintEngine");
...
function computeScoreLite(food) {
  ...
  // scoreLite is a lightweight heuristic for pre-ranking
  // rewards digestibility and penalizes rule violations
...
function generateCandidates(template, foods, userState, rules, options) {
  ...
  return categories.reduce((candidateMap, category) => {
    ...
    const filteredResult = filterFoods(preFilteredFoods, userState, safeRules);
    const validFoods = toSafeArray(filteredResult.validFoods);

    candidateMap[category] = validFoods
